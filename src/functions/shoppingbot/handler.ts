...
const { TELEGRAM_URI, CURR_LINK, CHAT_ID } = process.env;

let keyboardButton = new URLSearchParams('{"keyboard":[["USD","EUR","TRY"]],"resize_keyboard":true}').toString();
keyboardButton = '&reply_markup=' + keyboardButton.slice(0, keyboardButton.length-1);
...
function sendToUser(chat_id: number, text: string, replyMarkup?: string) {
  console.debug('replyMarkup: ', replyMarkup);
  return `${TELEGRAM_URI}/sendMessage?chat_id=${chat_id}&text=${text}${replyMarkup || ''}`
}
  ...
  if (text) {
    ...
    try {
      ...
      if (codes.some(code => code === text.toUpperCase())) {
        ...
        const responseMessage = `Name: ${data.name}%0ACurrency: ${data.rateFormated}%0ADate: ${data.date.slice(0, data.date.length-5).split('T').join(' ')}`;
        console.debug('date: ', data.date);
        console.debug('responseMessage: ', responseMessage);
        console.debug('chat.id: ', chat.id);
        await getRequest(sendToUser(chat.id, responseMessage, keyboardButton));
        if (chat.id !== Number(CHAT_ID)) {
          await getRequest(sendToUser(Number(CHAT_ID), responseMessage, keyboardButton));
        }
      } else {
        ...
        const response = await getRequest(sendToUser(chat.id, `Available codes: ${codes}`, keyboardButton));
